  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 22 }}>
      <span style={{ display: 'flex', alignItems: 'center', gap: 7, fontFamily: 'var(--font-syne)', fontWeight: 600, fontSize: 11, letterSpacing: '.3em', color: 'var(--gold2)' }}>
        ...
        {dateStr && <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 11, letterSpacing: '.08em', color: 'var(--text3)', fontWeight: 400 }}>{dateStr}</span>}
      </span>
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <ThemeToggle size="sm" />
        {/* Búsqueda — estilo pill gold para distinguirla */}
        <button onClick={openSearch} aria-label="Buscar"
          style={{ height: 32, padding: '0 12px', borderRadius: 999, border: '.5px solid var(--gold2)', background: 'transparent', display: 'flex', alignItems: 'center', gap: 5, color: 'var(--gold2)', cursor: 'pointer' }}
        >
          ...
          <span style={{ fontFamily: 'var(--font-dm-mono)', fontSize: 9, letterSpacing: '.12em' }}>BUSCAR</span>
        </button>
