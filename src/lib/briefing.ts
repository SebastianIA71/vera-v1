...
/**
 * Genera (o devuelve cacheado) el briefing matutino del día actual.
 * Se cachea en `memory` bajo la clave `morning_briefing_YYYY-MM-DD`.
 * Usado tanto por GET /api/briefing/morning (on-demand, con rate limit)
 * como por el cron de las 9h (force=true, regenera con datos frescos
 * ya que el ritual matutino ha terminado a esa hora).
 */
export async function generateMorningBriefing(force = false): Promise<{ briefing: string | null; cached: boolean }> {
  const now = new Date();
  ...
  if (!force) {
    ...
    if (cached?.value) {
      return { briefing: cached.value, cached: true };
    }
  }
...
${extraContext}`;
  ...
  if (!result.ok) {
    return { briefing: null, cached: false };
  }
  ...
  return { briefing: result.text, cached: false };
}
